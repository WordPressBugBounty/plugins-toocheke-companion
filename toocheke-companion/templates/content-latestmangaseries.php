<?php
    /**
 * Template part for displaying latest manga series
 ...
 */
    ...
    $series_order        = 'DESC';
    if (get_query_var('series_order')) {
    $series_order = get_query_var('series_order');
    }

    $single_manga_series_args = [
    ...
    $single_manga_series_query = new WP_Query($single_manga_series_args);
?>
<?php
    /* Start the Loop */
    global $post;

    while ($single_manga_series_query->have_posts()): $single_manga_series_query->the_post();

    $manga_series_id = $post->ID;
