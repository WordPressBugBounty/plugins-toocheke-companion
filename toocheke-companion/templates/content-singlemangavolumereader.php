<?php
    ...
    $series_permalink = get_permalink( $manga_series_id );
    $volumes_query = null;
    if ($manga_series_id) {
...
<div id="manga-page-top-nav" class="manga-page-nav">
    <a href="<?php echo esc_url($series_permalink);?>" title="Close reader" class="close-reader"><svg viewBox="0 0 96 96" xmlns="http://www.w3.org/2000/svg"><title></title><path d="M69.8437,43.3876,33.8422,13.3863a6.0035,6.0035,0,0,0-7.6878,9.223l30.47,25.39-30.47,25.39a6.0035,6.0035,0,0,0,7.6878,9.2231L69.8437,52.6106a6.0091,6.0091,0,0,0,0-9.223Z"></path></svg> Close reader</a>
</div>
   <!--./MANGA TOP NAV-->

                     <!--start content-->
                         <div class="manga-volume-container manga-reader-container two-pages">
                  
    <?php


    // Only show pages  if ?reader=true
        ...
        $chapters_query  = new WP_Query([
            ...
            'meta_query'     => [
                [
                    'key'     => 'volume_id', // adjust this meta key
        ...
        if ($chapters_query->have_posts()) {
            ...
            while ($chapters_query->have_posts()) {
                ...
                $chapter_imgs = get_post_meta(get_the_ID(), 'manga_chapter_pages', true);
                if (!empty($chapter_imgs) && is_array($chapter_imgs)) {
                    $all_images = array_merge($all_images, $chapter_imgs);
                }
            }
            wp_reset_postdata();
        }
    ...
    <div class="spinner"></div>
            ...
            <swiper-container id="manga-swiper"class="swiper" pagination="true" pagination-type="progressbar" navigation="true" space-between="0" slides-per-view="2" slides-per-group="2"  keyboard="true" events-prefix="swiper-" direction="<?php echo esc_attr($direction);?>" breakpoints='{"0":{"direction":"vertical","slidesPerView":1},"768":{"direction":"horizontal","slidesPerView":2}}'>
                    ...
                    <?php foreach ($all_images as $img_id): 
                        ...
                        <?php endif; ?>
                    <?php endforeach; ?>
<swiper-slide><a href="<?php echo esc_url($series_permalink);?>" class="btn-manga-close-reader btn btn-dark">Close Reader</a></swiper-slide>
               </swiper-container>
...
<div class="manga-nav-wrapper">
 <?php _e('Browse Volumes', 'toocheke-companion'); ?>
<?php 
 if ($volumes_query->have_posts()) : ?>
        <select onchange="document.location.href=this.options[this.selectedIndex].value" class="manga-dropdown">
            <?php while ($volumes_query->have_posts()) : $volumes_query->the_post(); ?>
                <option value="<?php echo esc_url(get_permalink() . '?reader=true'); ?>"
                    <?php selected(get_the_ID(), $manga_volume_id); ?>>
                    <?php the_title(); ?>
                </option>
            <?php endwhile; ?>
        </select>
