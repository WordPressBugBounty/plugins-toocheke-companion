...
<?php echo esc_attr($content); ?>
    ...
    <?php }
        ...
        <p><b>Remove this file?</b> <input type="checkbox" id="remove_comic_audio" name="remove_comic_audio" value="1"></p>
            ...
            {
                add_meta_box('comicscout-image-metabox', __('ComicScout Thumbnail', 'toocheke-companion'), [
                    $this,
                    'toocheke_comicscout_image_display_metabox'
                ], 'comic', 'side', 'high');
            }
...
public function toocheke_enqueue_manga_filter_script()
{
    wp_enqueue_script(
        ...
        plugin_dir_url(__FILE__) . 'js/manga-filter.js',
        [],
        TOOCHEKE_COMPANION_VERSION,
        true
    );
...
class Toocheke_Future_Comic_Image_Protection {
    ...
    private function __construct() {
        ...
        add_action( 'transition_post_status', [ $this, 'maybe_protect_on_schedule' ],      10, 3 );

        // Protect any image uploaded while the post is already in 'future' status.
        add_action( 'add_attachment',         [ $this, 'maybe_protect_uploaded_image' ],   10, 1 );

        // When a comic transitions to 'publish', restore all protected images.
        add_action( 'transition_post_status', [ $this, 'maybe_restore_image_on_publish' ], 10, 3 );

        // Fallback for WP-Cron scheduled publishing.
        add_action( 'publish_comic',          [ $this, 'restore_on_publish_cron' ],        10, 2 );

        // Clean up private files if an attachment is force-deleted externally.
        add_action( 'delete_attachment',      [ $this, 'cleanup_private_file' ],           10, 1 );

        // Admin notice warning editors when a comic is already scheduled.
        add_action( 'admin_notices',          [ $this, 'scheduled_comic_admin_notice' ] );
    }
     ...
     */
    public function maybe_protect_on_schedule( string $new_status, string $old_status, \WP_Post $post ): void {
        // Only fire when post becomes 'future' for the first time.
        ...
        if ( empty( $attachments ) ) {
            //error_log( 'Toocheke: no attachments found to protect for comic ' . $post->ID );
            return;
        }
    ...
    /**
     * Fires after an attachment is added to the DB.
     * If it is uploaded while the parent post is already scheduled,
     * immediately move it to private storage.
     */
    public function maybe_protect_uploaded_image( int $attachment_id ): void {
        ...
        $this->protect_attachment( $parent_id, $attachment_id );
    }
    ...
    private function protect_attachment( int $parent_id, int $attachment_id ): void {
        ...
        // Set flag so cleanup_private_file knows to skip this deletion.
     ...
     */
    public function maybe_restore_image_on_publish( string $new_status, string $old_status, \WP_Post $post ): void {
     ...
     */
    public function restore_on_publish_cron( int $post_id, \WP_Post $post ): void {
     ...
     * ourselves during protect_attachment.
     */
    public function cleanup_private_file( int $attachment_id ): void {
        // Skip if we triggered this deletion ourselves during protect_attachment.
        if ( $this->protecting ) {
            return;
        }
     ...
     */
    public function scheduled_comic_admin_notice(): void {
        $screen = get_current_screen();
